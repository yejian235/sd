...
elseif ($_REQUEST['act'] == 'list_back')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('list_back.htm');
}
/*------------------------------------------------------ */
//-- 退货详情
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'back_detail')
{
    /* 检查权限 */
    admin_priv('back_view');
    $back_sn = $_REQUEST['back_sn'];
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('back_detail.htm');
}
/*------------------------------------------------------ */
//-- 通过审核
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'pase_back')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('back_pase.htm');
}
/*------------------------------------------------------ */
//-- 通过审核提交
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'pase_back_submit')
{
    /* 检查权限 */
    admin_priv('back_view');
    $back_sn = $_REQUEST['back_sn'];
...
//-- 重新通过审核
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'repase_back')
{
    /* 检查权限 */
    admin_priv('back_view');
    $back_sn = $_REQUEST['back_sn'];
    $sql = "delete from " .$ecs->table('order_back'). " where back_sn='$back_sn'";
    //$sql = "update " .$ecs->table('order_back'). " set status = 2 where back_sn='$back_sn'";
    $db->query($sql);
    ecs_header("Location: order.php?act=list_back\n");
    exit;
}
...
//-- 确认退款
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'back_tuikuan')
{
    /* 检查权限 */
    admin_priv('back_view');
    $back_sn = $_REQUEST['back_sn'];
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('back_tuikuan.htm');
}
/*------------------------------------------------------ */
//-- 确认退款提交
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'back_money')
{
    ...
    $insert = "INSERT INTO " .$ecs->table('user_account'). " VALUES ('', '$user_id', '$_SESSION[admin_name]','$back_money', '".gmtime()."', '".gmtime()."', '$goods', '$goods', '0', '余额支付', '1')";
    $db->query($insert);
    // 更新会员余额数量
    ...
    $update = "update " .$ecs->table('order_back'). " set status = 5 where back_sn='$back_sn'";
    $db->query($update);
    /* 记录log */
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' . " VALUES ('" . gmtime() . "', $_SESSION[admin_id], '退款：退换货编号 " . $back_sn . " 成功退款 " . $back_money . " 元', '" . real_ip() . "')";
    $db->query($sql);
    ecs_header("Location: order.php?act=list_back\n");
    exit;
}
...
elseif ($_REQUEST['act'] == 'stop_back')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('back_stop.htm');
}
/*------------------------------------------------------ */
//-- 取消申请提交
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'stop_back_submit')
{
    /* 检查权限 */
    admin_priv('back_view');
    $back_sn = $_REQUEST['back_sn'];
...
/* 退换货留言提交 */
elseif ($_REQUEST['act'] == 'back_message')
{
    $back_sn = $_POST['back_sn'];
    $message = $_POST['message'];
    $add_time = date('Y-m-d H:i:s',time());
    $sql = "INSERT INTO " .$ecs->table('back_message'). " (message, add_time, back_sn, is_admin) values ('$message', '$add_time', '$back_sn', 1)";
    $db->query($sql);
    $links[] = array('text' => '退货列表', 'href' => 'order.php?act=list_back');
    sys_msg('添加回复成功', 1, $links);
    exit;
}
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
		...
		/* 商家发货时给客户发送短信提醒 */
		if ($_CFG['ecsdxt_order_shipped'] == '1' && $row['mobile_phone'] != '')
		{
