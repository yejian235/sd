...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','send_pwd_mobile','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name','mpassword_name', 'get_passwd_question', 'check_answer','oath' , 'oath_login', 'other_login');
...
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'mpassword_name','get_passwd_question', 'check_answer','bindmobile');
...
/* add by mofangweb.com 退换货插件 加强版 begin */
/* 申请换货 */
...
/* 寄送退货商品 */
elseif ($action == 'tuihuo')
{
    $back_sn = isset($_GET['back_sn']) ? intval($_GET['back_sn']) : 0;
    $smarty->assign('back_sn', $back_sn);
    $smarty->display('user_transaction.dwt');	
}
/* 提交退货配送信息表单 */
elseif ($action == 'shenqing_submit')
{
    ...
    if((isset($_FILES['back_pic1']['error']) && $_FILES['back_pic1']['error'] == 0) || (!isset($_FILES['back_pic1']['error']) && isset($_FILES['back_pic1']['tmp_name']) && $_FILES['back_pic1']['tmp_name'] != 'none'))
    {
        if($_FILES['back_pic1']['size'] / 1024 > $upload_size_limit)
        {
            show_message(sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit));
            exit;
        }
        // 检查文件格式
        if (!check_file_type($_FILES['back_pic1']['tmp_name'], $_FILES['back_pic1']['name'], $allow_file_types))
        {
            show_message("只能上传GIF,JPG,JPEG,PNG,BMP的图片！");
        }
    }
    if((isset($_FILES['back_pic2']['error']) && $_FILES['back_pic2']['error'] == 0) || (!isset($_FILES['back_pic2']['error']) && isset($_FILES['back_pic2']['tmp_name']) && $_FILES['back_pic2']['tmp_name'] != 'none'))
    {
        if($_FILES['back_pic2']['size'] / 1024 > $upload_size_limit)
        {
            show_message(sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit));
            exit;
        }
        // 检查文件格式
        if (!check_file_type($_FILES['back_pic2']['tmp_name'], $_FILES['back_pic2']['name'], $allow_file_types))
        {
            show_message("只能上传GIF,JPG,JPEG,PNG,BMP的图片！");
        }
    }
    if((isset($_FILES['back_pic3']['error']) && $_FILES['back_pic3']['error'] == 0) || (!isset($_FILES['back_pic3']['error']) && isset($_FILES['back_pic3']['tmp_name']) && $_FILES['back_pic3']['tmp_name'] != 'none'))
    {
        if($_FILES['back_pic3']['size'] / 1024 > $upload_size_limit)
        {
            show_message(sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit));
            exit;
        }
        // 检查文件格式
        if (!check_file_type($_FILES['back_pic3']['tmp_name'], $_FILES['back_pic3']['name'], $allow_file_types))
        {
            show_message("只能上传GIF,JPG,JPEG,PNG,BMP的图片！");
        }
    }
    $back_pic1 = $image->upload_image($_FILES['back_pic1'], 'backimg');
    $back_pic2 = $image->upload_image($_FILES['back_pic2'], 'backimg');
    $back_pic3 = $image->upload_image($_FILES['back_pic3'], 'backimg');
    ...
    $db->query($sql);
    show_message('您的售后申请已提交，客服将按顺序依次处理，一般当天即可处理&hellip;<br>退货地址见退款详情页管理员回复处。<br>退款将退还账户余额。', '返回列表页', 'user.php?act=order_back','info');
    exit;
}
/* 提交退货配送信息表单 */
...
/* 退换货搜索列表 */
elseif ($action == 'order_back_search')
{
    $back_sn = $_REQUEST['back_sn'];
    $invoice_no = $_REQUEST['invoice_no'];
...
/* 退换货留言提交 */
elseif ($action == 'back_message')
{
    $back_sn = $_POST['back_sn'];
    $message = $_POST['message'];
    $add_time = date('Y-m-d H:i:s',time());
    $sql = "INSERT INTO " .$ecs->table('back_message'). " (message, add_time, back_sn, is_admin) values ('$message', '$add_time', '$back_sn', 0)";
    $db->query($sql);
    show_message('您的留言已提交，等待管理员回复。', '返回列表页', 'user.php?act=order_back','info');
    exit;
}
/* add by mofangweb.com 退换货插件 加强版 end */
/* 合并订单 */
