...
class EcsSqlExecutor {
    ...
    function query($query_item) {
        ...
        /* 处理ALTER TABLE语句，此时程序将对表的结构进行修改 */ elseif ($this->auto_match && preg_match('/^\s*ALTER\s+TABLE\s*/i', $query_item)) {
            if (!$this->alter_table($query_item)) {
                return false;
            }
        }
        /* 处理其它修改操作，如数据添加、更新、删除等 */ else {
    ...
    function create_table($query_item) {
        ...
        /* 重新设置表属性声明串 */
        $postfix = $this->db->version() > '4.1' ? " ENGINE=$type DEFAULT CHARACTER SET " . $this->db_charset : " TYPE=$type";
        $postfix .= ' ' . $auto_incr;
    ...
    function parse_change_query($query_item, $table_name = '') {
        ...
        if (preg_match_all($pattern, $query_item, $matches, PREG_SET_ORDER)) {
            ...
            for ($i = 0; $i < $num; $i++) {
                ...
                /* 如果表中存在新列名 */ elseif (in_array($matches[$i][2], $fields)) {
    ...
    function parse_drop_index_query($query_item, $table_name = '') {
        ...
        if (preg_match_all($pattern, $query_item, $matches, PREG_SET_ORDER)) {
            ...
            for ($i = 0; $i < $num; $i++) {
                ...
                /* 否则删除索引 */ elseif (in_array($matches[$i][1], $indexes)) {
