    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        /*$arr[] = array('order_id'       => $row['order_id'],
		 ...
		 $arr[] = array('order_id'       => $row['order_id'],
				   ...
				   'order_time'     => local_date($GLOBALS['_CFG']['time_format'], $row['add_time']),
				   'order_status'   => $row['order_status'],
				   'shipping_status'   => $row['shipping_status'], /* add by mofangweb.com 退换货插件 加强版 */
				   'pay_status'     => $row['pay_status'], /* add by mofangweb.com 退换货插件 加强版 */
				   'back_status'    => $GLOBALS['db']->getOne("SELECT status FROM " . $GLOBALS['ecs']->table('order_back') . " WHERE order_sn = '" . $row['order_sn'] . "'"), /* add by mofangweb.com 退换货插件 加强版 */
				   'back_case'      => $GLOBALS['db']->getOne("SELECT `case` FROM " . $GLOBALS['ecs']->table('order_back') . " WHERE order_sn = '" . $row['order_sn'] . "'"), /* add by mofangweb.com 退换货插件 加强版 */
				   'back_sn'        => $GLOBALS['db']->getOne("SELECT back_sn FROM " . $GLOBALS['ecs']->table('order_back') . " WHERE order_sn = '" . $row['order_sn'] . "'"), /* add by mofangweb.com 退换货插件 加强版 */
				   'diff_time'      => time()-8*3600 - $row['add_time'], /* add by mofangweb.com 退换货插件 加强版 */
				   'total_fee'      => price_format($row['total_fee'], false),
				   'handler'        => $row['handler']);
    }
...
function get_user_orders_back_search($user_id, $num = 10, $start = 0, $back_sn, $invoice_no, $order_sn)
{
    /* 取得订单列表 */
    $arr = array();
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('order_back') . " WHERE user_id = '$user_id'";	   
    if($back_sn)
    {
