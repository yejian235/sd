	...
	if (!empty($_COOKIE['ECS']['history']))
    {
		...
		if(!empty($arr[0]))
		{
			...
			$goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
			 ...
			 $str .= '<li><h2><a class="track" target="_blank" href="'.$goods[$idx]['url'].'"><img src="'.$goods[$idx]['thumb'].'" width="166" height="166" style="display: inline;"></a></h2><span class="rchannel_blank15"></span><h4><a class="track" target="_blank" href="'.$goods[$idx]['url'].'">'.$goods[$idx]['short_name'].'</a></h4><span class="rchannel_blank5"></span><p class="rchannel_productPrice"><span class="rchannel_scPrice">市场价￥<del'.$goods[$idx]['market_price'].'</del></span><span class="rchannel_salePrice"><strong>售价'.$goods[$idx]['shop_price'].'</strong></span></p></li>';
			}
		}
		return $str;
	}
...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.=' <dl class="rchannel_clearfix"><dt><a class="track" href="'.$goods['url'].'" target="_blank"><img style="width: 68px; height: 68px; display: inline;" src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" /></a></dt><dd><a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a></dd><p>'.$goods['shop_price'].'</p> </dl>';
        }
 ...
 * @return  string
 */
function insert_history_search()
{
    $str = '';
    if (!empty($_COOKIE['ECS']['history']))
    {
      ...
      <div>
        <div class="wyimg"><a class="track" target="_blank" href="'.$goods['url'].'"><img style="width: 68px; height: 68px; display: inline;" alt="'.$goods['goods_name'].'" src="'.$goods['goods_thumb'].'"></a></div>
        <div class="wytitle">
          <div class="wytitle1"><a title="'.$goods['goods_name'].'" target="_blank" class="track" href="'.$goods['url'].'">'.$goods['goods_name'].'</a></div>
          <div class="wyshoujia">本店价：'.$goods['shop_price'].'</div>
        </div>
      </div>
    </li></ul>';
        }
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
          ...
          if ($row['promote_price'] > 0)
            ...
            $goods['url'] = build_uri('goods', array('gid'=>$row['goods_id']), $row['goods_name']);
            $str.='<ul>
          <li class="topImg1"><a class="track" target="_blank" href="'.$goods['url'].'"> <img alt="'.$goods['goods_name'].'" src="'.$goods['goods_thumb'].'" style="display: inline;"> </a></li>
          <li class="topWord1"><span class="blank15"></span>
            <p class="FBListPruName"> <a target="_blank" class="track" href="'.$goods['url'].'">'.$goods['short_name'].'</a></p>
            <span class="blank5"></span>
            <p class="Sprice"> 售价：'.$goods['shop_price'].'</p>
            <span class="blank5"></span></li>
        </ul><span class="blank5"></span>';
        }
